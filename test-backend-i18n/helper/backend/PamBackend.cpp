...
namespace SDDM {
    ...
    AuthPrompt::Type PamData::detectPrompt(const struct pam_message* msg) const {
        if(m_workState.value == PamWorkState::STATE_AUTHENTICATE) {
            if (msg->msg_style == PAM_PROMPT_ECHO_OFF)
                return AuthPrompt::LOGIN_PASSWORD;
            else if (msg->msg_style == PAM_PROMPT_ECHO_ON)
                return AuthPrompt::LOGIN_USER;
        }
        else if(m_workState.value == PamWorkState::STATE_CHANGEAUTHTOK) {
            if (msg->msg_style == PAM_PROMPT_ECHO_OFF)
        ...
        if (predict) {
            ...
            switch (type) {
                ...
                case AuthPrompt::CHANGE_PASSWORD:
                    m_currentRequest = Request( { { type, QString::fromLocal8Bit(msg->msg), type == AuthPrompt::LOGIN_USER ? true : false} } );
                    return true;
                default:
                    break;
            }
    ...
    */
    ...
    bool PamBackend::authenticate() {
        ...
        LOG_FCN_CALL(PamBackend, authenticate);
    ...
    bool PamBackend::openSession() {
        LOG_FCN_CALL(PamBackend, openSession);
    ...
    /** what to do when pam_chauthtok (password expired) fails
     * with PAM_MAXTRIES: continue in loop or give up and return? */
    void PamBackend::setChAuthTokLoop(bool loop) {
        m_pam->setMaxTriesLoop(loop);
    }
    ...
    int PamBackend::converse(int n, const struct pam_message **msg, struct pam_response **resp) {
        ...
        *resp = (struct pam_response *) calloc(n, sizeof(struct pam_response));
        ...
        for (int i = 0; i < n; i++) {
            ...
            // show responses
            qDebug() << "[PAM] pam_conv: resp[" << i << "] =" << response;
